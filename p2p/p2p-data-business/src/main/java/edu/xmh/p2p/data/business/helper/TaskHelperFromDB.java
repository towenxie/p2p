...
import edu.xmh.p2p.data.business.view.SearchTableView;
...
public class TaskHelperFromDB {
    ...
    // 新建下载任务，开始下载
    public static void createTask(FileInfoEntity filenInfo) {
        DispatcherApp dispatcher = DispatcherApp.appServiceFormDB(filenInfo);
        TaskHelperFromDB.append(dispatcher);
    }
    ...
    public static int canAddTask(String fileName) {
        ...
        for (int i = 0; i < taskNum; i++) {
            if (tasks[i] == null) {
                result = 1;
            } else if (tasks[i] != null && fileName.equals(tasks[i].fileName)) {
                result = 2;
                break;
            }
        }
    ...
    public static void setUpAllTasks() {
        ...
        for (String fileName : filelist) {
            // 新建下载任务，开始下载
            FileInfoEntity filenInfo = SearchTableView.pickTableData(fileName);
            createTask(filenInfo);
        }
    }
    ...
    public static void saveTask() {
        for (int i = 0; i < taskNum; i++) {
            if (tasks[i] != null && tasks[i].downOver) {
                FileHelper.write(tasks[i].fileName);
            }
        }
    }
    ...
    public static void continueDownload(String name) {
        for (int i = 0; i < taskNum; i++) {
            if (tasks[i] != null && name.equals(tasks[i].dataName)) {
                if (tasks[i].stop) {
                    FileInfoEntity filenInfo = SearchTableView.pickTableData(tasks[i].fileName);
                    DispatcherApp dispatcher = DispatcherApp.appServiceFormDB(filenInfo);
                    tasks[i] = dispatcher;
                } else {
